...
public class MountainCar implements DomainGenerator {
	...
	public State move(State s, int dir){
		...
		System.out.println("Mountain Car: "+p0+" "+v0+" "+p1+" "+v1);
		Observation.setX((int)(p1*100));
		Observation.setV((int)(v1*100));
...
//		exp.addKeyAction("a", ACTIONBACKWARDS);	
		...
		while(true) {
			System.out.println("");
			if(Observation.getFlag()==1) {
				System.out.println("Mountain Car: PyRL -"+Observation.getX()+" "+Observation.getV());
			
				exp.takeAction(RLAction());
				Observation.setFlag(0);
			}
		}
	...
	private static String RLAction() {
		System.out.println("Mountain Car: "+Observation.getAction());
		switch (Observation.getAction()) {
			case -1:
				return ACTIONBACKWARDS;
			case 0:
				return ACTIONCOAST;
			case 1:
